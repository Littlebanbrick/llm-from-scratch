...
car_volume = torch.rand(100, 1) * 1.8 + 0.5
car_weight = torch.rand(100, 1) * 1.5 + 0.5
car_data = torch.cat([car_volume, car_weight], dim=1)
car_label = torch.zeros(100, 1)

# Truck: volume 2.0~5.0, weight 1.5~4.0
truck_volume = torch.rand(100, 1) * 3.0 + 2.0
truck_weight = torch.rand(100, 1) * 2.5 + 1.5
...
# ---------------------------
def train_with_validation(X_train, y_train, X_val, y_val, epochs=500, patience=20):
    model = Classifier()
...
model = train_with_validation(X_train, y_train, X_val, y_val, epochs=500, patience=20)
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(X_trainval_t)):
    ...
    model_fold = Classifier()
    criterion = nn.BCELoss()
    optimizer = optim.Adam(model_fold.parameters(), lr=0.01)
...
def plot_decision_boundary(model, mean, std):
    ...
    plt.xlabel('Volume')
    plt.ylabel('Weight')
    plt.title('Decision Boundary (Probability of Truck)')
